...
export default defineCommand({
  ...
  run: async ({ message, prisma, redis }) => {
    const buttonRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId('accept').setStyle(ButtonStyle.Success).setLabel('Accept'),
      new ButtonBuilder().setCustomId('decline').setStyle(ButtonStyle.Danger).setLabel('Decline')
    );
